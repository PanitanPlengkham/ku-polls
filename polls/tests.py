...
def create_question(question_text, days):
    ...
    return Question.objects.create(question_text=question_text,
                                   pub_date=time, end_date=time)


class QuestionModelTests(TestCase):
    """Class for Question model test."""

    def test_was_published_recently_with_future_question(self):
        """To returns False for questions whose pub_date is in the future."""
        time = timezone.now() + datetime.timedelta(days=30)
        future_question = Question(pub_date=time)
        self.assertIs(future_question.was_published_recently(), False)

    def test_was_published_recently_with_old_question(self):
        """To returns False whose pub_date is older than 1 day."""
        time = timezone.now() - datetime.timedelta(days=1, seconds=1)
        old_question = Question(pub_date=time)
        self.assertIs(old_question.was_published_recently(), False)

    def test_was_published_recently_with_recent_question(self):
        """To returns True whose pub_date is within the last day."""
    ...
    def test_can_vote_with_future_question(self):
        """To return false when polls are not open yet."""
        time = timezone.now() + datetime.timedelta(days=2)
...
class QuestionDetailViewTests(TestCase):
    """Class for Question detail view test."""
